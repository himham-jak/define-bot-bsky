    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (currentSubstring.length >= length) {
            ...
            const substringFacets = facets.reduce((acc, facet) => {
                ...
                if (byteStart >= byteOffset && byteEnd <= byteOffset + substringByteLength + 1 // byteEnd is exclusive, so if it's at the end of a string, it will be 1 greater than the last index
                ) {
                    acc.push({
                        ...facet,
                        index: {
